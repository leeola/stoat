...
/// Helper methods for SimpleText syntax
impl SyntaxNode<SimpleText> {
    /// Find the line node containing the given offset
    pub fn find_line_at_offset(&self, offset: TextSize) -> Option<SyntaxNode<SimpleText>> {
        // Start from the deepest node and work up to find a line
        let mut current = self.find_node_at_offset(offset)?;

        loop {
            if current.kind() == SimpleKind::Line {
                return Some(current);
            }
    ...
    /// Get all line nodes in order
    pub fn lines(&self) -> Vec<SyntaxNode<SimpleText>> {
        let mut lines = Vec::new();
        self.collect_lines(&mut lines);
        lines
    }

    fn collect_lines(&self, lines: &mut Vec<SyntaxNode<SimpleText>>) {
        for child in self.children() {
            match child {
                SyntaxElement::Node(node) => {
                    if node.kind() == SimpleKind::Line {
                        lines.push(node.clone());
                    } else {
                        node.collect_lines(lines);
                    }
                },
    ...
    /// Find next word boundary from the given offset
    pub fn find_next_word_boundary(&self, offset: TextSize) -> Option<TextSize> {
        let tokens = self.tokens();

        for token in &tokens {
            if token.kind() == SimpleKind::Word && token.text_range().start() > offset {
                return Some(token.text_range().start());
            }
        }

        // No more words, return end of document
        Some(self.text_range().end())
    }

    /// Find previous word boundary from the given offset
    pub fn find_prev_word_boundary(&self, offset: TextSize) -> Option<TextSize> {
        let tokens = self.tokens();

        // Find the last word that starts before the offset
        let mut result = Some(TextSize::from(0));
        for token in &tokens {
            if token.kind() == SimpleKind::Word && token.text_range().start() < offset {
                result = Some(token.text_range().start());
            } else if token.text_range().start() >= offset {
                break;
            }
        }
