...
/// Increment a number literal's text by `delta`, preserving its radix,
/// digit case, and underscore grouping. `text` is the captured literal
/// (e.g. the range from [`find_number_at`]); `kind` selects the radix.
/// Returns `None` when the body has no parseable digits. Saturates at
/// the integer bounds rather than wrapping.
