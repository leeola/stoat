...
//!
//! Renders the command palette modal overlay based on state from [`stoat_v4::Stoat`]. This
...
/// [`stoat_v4::Stoat`]. All interaction is handled through the normal action system in
/// command_palette mode:
///
/// - Text input goes to the input buffer via [`stoat_v4::Stoat::insert_text`]
/// - Backspace deletes from input buffer via [`stoat_v4::Stoat::delete_left`]
/// - Arrow keys navigate via
///   [`stoat_v4::Stoat::command_palette_next`]/[`stoat_v4::Stoat::command_palette_prev`]
/// - Escape dismisses via [`stoat_v4::Stoat::command_palette_dismiss`]
/// - Enter executes via command_palette_execute handler in GUI
///
/// The command palette is displayed when [`stoat_v4::Stoat::mode`] returns `"command_palette"`.
#[derive(IntoElement)]
