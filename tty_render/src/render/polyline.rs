...
impl PolylinePass {
    ...
    pub(crate) fn new(
        ...
        let occluders = OccluderBuffer::new(device, "polyline occluders", INITIAL_CAPACITY);
        let composite_occluders =
            OccluderBuffer::new(device, "polyline composite occluders", INITIAL_CAPACITY);
    ...
    ///
    /// The paths are occluded against `occluders` with the seq test bypassed, so
    /// a line gliding beneath a modal is hidden by it. `occluders` carries the
