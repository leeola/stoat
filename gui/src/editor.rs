...
impl Editor {
    pub fn new(
        ...
        let font_size_sub = cx.observe_global::<FontSizeOffset>(|_, cx| cx.notify());
...
/// Minimum and maximum editor buffer font size in points. The
/// configured base plus the runtime [`FontSizeOffset`] is clamped to
/// this range, mirroring Zed's 6px-100px buffer-font bounds.
pub(crate) const MIN_EDITOR_FONT_SIZE: f32 = 6.0;
pub(crate) const MAX_EDITOR_FONT_SIZE: f32 = 100.0;

/// Effective editor buffer font size: the configured `editor.font.size`
/// (or [`DEFAULT_EDITOR_FONT_SIZE`]) plus the session [`FontSizeOffset`],
/// clamped to [`MIN_EDITOR_FONT_SIZE`]..=[`MAX_EDITOR_FONT_SIZE`].
pub(crate) fn editor_font_size(cx: &App) -> f32 {
    ...
    (configured + offset).clamp(MIN_EDITOR_FONT_SIZE, MAX_EDITOR_FONT_SIZE)
}

/// Step the session [`FontSizeOffset`] by `delta` points, so every
/// editor observing the global re-lays-out with the configured base
/// shifted by the new offset.
pub(crate) fn adjust_font_size(delta: f32, cx: &mut App) {
    let offset = cx.try_global::<FontSizeOffset>().map_or(0.0, |o| o.0);
    cx.set_global(FontSizeOffset(offset + delta));
}
...
mod tests {
    ...
    #[test]
    fn adjust_font_size_steps_offset_and_clamps() {
        let cx = TestAppContext::single();
        cx.update(|cx| {
            assert_eq!(editor_font_size(cx), DEFAULT_EDITOR_FONT_SIZE);

            adjust_font_size(1.0, cx);
            assert_eq!(editor_font_size(cx), DEFAULT_EDITOR_FONT_SIZE + 1.0);

            cx.set_global(FontSizeOffset(5.0));
            assert_eq!(
                editor_font_size(cx),
                DEFAULT_EDITOR_FONT_SIZE + 5.0,
                "offset rides on top of the base"
            );

            cx.set_global(FontSizeOffset(MAX_EDITOR_FONT_SIZE));
            adjust_font_size(5.0, cx);
            assert_eq!(
                editor_font_size(cx),
