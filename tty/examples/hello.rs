...
/// Build `example_hello_app` and return the path to the compiled binary.
///
...
fn build_emitter() -> PathBuf {
    ...
    assert!(status.success(), "building example_hello_app failed");

    profile_dir.join("example_hello_app")
}
