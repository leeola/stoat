...
#[test]
fn stash_create_returns_sha_when_dirty() {
    let tr = TestRepo::new();
    tr.commit_file("a.rs", "v1\n");
    ...
    let sha = repo.stash_create().expect("dirty workdir produces stash");

    let oid = git2::Oid::from_str(&sha).expect("sha is a valid oid");
    let stash_commit = tr.repo.find_commit(oid).expect("stash commit object");
    let stash_tree = stash_commit.tree().expect("stash tree");
    let entry = stash_tree
        .get_path(Path::new("a.rs"))
        .expect("a.rs in tree");
    let blob = entry
        .to_object(&tr.repo)
        .expect("blob object")
        .peel_to_blob()
        .expect("peel to blob");
    assert_eq!(blob.content(), b"v2-uncommitted\n");
}

#[test]
fn stash_create_returns_none_when_clean() {
    let tr = TestRepo::new();
    tr.commit_file("a.rs", "v1\n");

    let host = LocalGit::new();
    let repo = host.discover(tr.path()).unwrap();
    assert_eq!(repo.stash_create(), None);
}
...
fn stash_create_does_not_modify_on_disk_index() {
    ...
    tr.commit_file("a.rs", "v1\n");
    tr.write("a.rs", "v2\n");
    tr.write("untracked.rs", "fresh\n");

    let host = LocalGit::new();
    let repo = host.discover(tr.path()).unwrap();
    let _ = repo.stash_create().expect("dirty workdir produces stash");

    let on_disk = Repository::open(tr.path()).expect("reopen repo");
    let mut opts = git2::StatusOptions::new();
    opts.include_untracked(true);
    let statuses = on_disk.statuses(Some(&mut opts)).expect("statuses");
    let observed: Vec<(String, git2::Status)> = statuses
        .iter()
    ...
    assert_eq!(
        observed,
        vec![
            ("a.rs".to_string(), git2::Status::WT_MODIFIED),
            ("untracked.rs".to_string(), git2::Status::WT_NEW),
        ],
        "stash_create must leave the index status unchanged",
    );
}
