...
impl DisplayMap {
    ...
    pub fn snapshot(&self) -> DisplaySnapshot {
        ...
        let tab_snapshot = self.tab_map.snapshot(&buffer_snapshot);
...
impl DisplaySnapshot {
    ...
    pub fn buffer_to_display(&self, point: Point) -> DisplayPoint {
        let tab = self.tab_snapshot.to_tab_point(point);
    ...
    pub fn display_to_buffer(&self, point: DisplayPoint) -> Option<Point> {
        ...
        let tab_point = TabPoint::new(point.row, point.column);
        let collapsed = self.tab_snapshot.to_buffer_point(tab_point, Bias::Left);
        Some(Point::new(buffer_point.row, collapsed.column))
    }
