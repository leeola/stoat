...
impl TestHarness {
    ...
    /// commit or notification produced by those wake-ups is routed
    /// through the main dispatch path before returning.
    pub fn advance_clock(&mut self, duration: Duration) {
        self.scheduler.advance_clock(duration);
        self.settle();
    }

    /// Drive the scheduler to a fixed point. After returning, every
    /// spawned task has been polled to suspension and every queued
    /// notification has been routed through the main dispatch path.
