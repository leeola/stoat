...
pub fn dispatch(stoat: &mut Stoat, action: &dyn Action) -> UpdateEffect {
    match action.kind() {
        ...
        ActionKind::OpenCommits => open_commits(stoat),
        ActionKind::CloseCommits => close_commits(stoat),
        ActionKind::CommitsNext => commits_step(stoat, CommitStep::Down(1)),
        ActionKind::CommitsPrev => commits_step(stoat, CommitStep::Up(1)),
        ActionKind::CommitsPageDown => commits_step(stoat, CommitStep::PageDown),
        ActionKind::CommitsPageUp => commits_step(stoat, CommitStep::PageUp),
...
fn open_commits(stoat: &mut Stoat) -> UpdateEffect {
...
fn close_commits(stoat: &mut Stoat) -> UpdateEffect {
    let ws = stoat.active_workspace_mut();
...
fn commits_step(stoat: &mut Stoat, step: CommitStep) -> UpdateEffect {
    let moved = {
...
fn commits_refresh(stoat: &mut Stoat) -> UpdateEffect {
...
/// Kick off another page load when the cursor is approaching the tail
/// of the loaded window. No-op when a load is already in flight or the
/// walk has hit a root commit.
...
/// Spawn a background preview build for the current selection if one
/// is not already cached or in flight. Also populates the file-change
...
/// lands we can request a preview for the selected commit). Returns
/// true when any task landed or a new task was spawned.
///
/// Called at the top of every `Stoat::render` tick so the UI reflects
/// settled state without requiring navigation input. Also called in
/// the test harness's `settle` loop so `assert_snapshot` sees terminal
/// state regardless of how many scheduler ticks the work needs.
