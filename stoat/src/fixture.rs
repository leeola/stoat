...
pub(in crate::fixture) struct FixtureRepo {
    repo: Repository,
    commit_count: i64,
}

impl FixtureRepo {
    pub(in crate::fixture) fn init(dest: &Path) -> Result<Self, FixtureError> {
    ...
    pub(in crate::fixture) fn commit(
        &mut self,
        message: &str,
        files: &[(&str, &str)],
    ...
    /// against HEAD.
    pub(in crate::fixture) fn staged_file(
        &mut self,
        name: &str,
        content: &str,
    ) -> Result<&mut Self, FixtureError> {
    ...
    pub(in crate::fixture) fn unstaged_file(
        &mut self,
        name: &str,
        content: &str,
    ) -> Result<&mut Self, FixtureError> {
    ...
    /// against the index.
    pub(in crate::fixture) fn unstaged_delete(
        &mut self,
        name: &str,
    ) -> Result<&mut Self, FixtureError> {
        let path = self.workdir().join(name);
