...
//!
//! ```ignore
//! use crate::test_harness::cursor_notation;
//!
//! // Cursor only
...
#[derive(Debug, Snafu)]
pub enum ParseError {
    #[snafu(display("selection missing cursor marker (use <||text|> or <|text||>)"))]
    SelectionMissingCursor {
        #[snafu(implicit)]
        location: snafu::Location,
    },

    #[snafu(display("cursor must be at selection boundary, not in middle"))]
    CursorNotAtBoundary {
        #[snafu(implicit)]
        location: snafu::Location,
    },

    #[snafu(display("unclosed selection (missing |>)"))]
    UnclosedSelection {
        #[snafu(implicit)]
        location: snafu::Location,
    },

    #[snafu(display("unexpected selection end |> without matching <|"))]
    UnexpectedSelectionEnd {
        #[snafu(implicit)]
        location: snafu::Location,
    },

    #[snafu(display("selection has cursor at both start and end"))]
    CursorAtBothEnds {
        #[snafu(implicit)]
        location: snafu::Location,
    },
            ...
            '|' => {
                ...
                match chars.peek() {
                    Some('|') => {
                        ...
                        if chars.peek() == Some(&'>') {
                            ...
                            if let Some((start, cursor_at_start)) = active_selection.take() {
                                if cursor_at_start {
                                    return CursorAtBothEndsSnafu.fail();
                                }
                                ...
                                return UnexpectedSelectionEndSnafu.fail();
                            }
                        } else {
                            // This is || - escaped pipe (outside selection)
                            if active_selection.is_some() {
                                return CursorNotAtBoundarySnafu.fail();
                            }
                            text.push('|');
                            byte_offset += 1;
                        }
                    ...
                    Some('>') => {
                        ...
                        if let Some((start, cursor_at_start)) = active_selection.take() {
                            if !cursor_at_start {
                                return SelectionMissingCursorSnafu.fail();
                            }
                            selections.push(Selection {
                                range: start..byte_offset,
            ...
            '<' => {
                ...
                if chars.peek() == Some(&'|') {
                    ...
                    if peek_iter.peek() == Some(&'|') {
                        ...
                        chars.next(); // consume second |
                        if active_selection.is_some() {
                            return UnclosedSelectionSnafu.fail();
                        }
...
mod tests {
    ...
    fn parse_multiple_selections() {
        ...
        assert_eq!(p.selections[0].range, 0..1);
        assert_eq!(p.selections[1].range, 2..3);
        assert!(!p.selections[0].cursor_at_start);
        assert!(!p.selections[1].cursor_at_start);
    }

    #[test]
    fn parse_mixed_cursors_and_selections() {
        let p = parse("|x <||y|> z|").unwrap();
        assert_eq!(p.text, "x y z");
        assert_eq!(p.cursors, vec![0, 5]);
        assert_eq!(p.selections.len(), 1);
        assert_eq!(p.selections[0].range, 2..3);
        assert!(p.selections[0].cursor_at_start);
    }
    ...
    fn error_unclosed_selection() {
        ...
        assert!(matches!(err, ParseError::UnclosedSelection { .. }));
    }

    #[test]
    fn error_unexpected_selection_end() {
        let err = parse("hello|>").unwrap_err();
        assert!(matches!(err, ParseError::UnexpectedSelectionEnd { .. }));
    }

    #[test]
    fn error_cursor_in_middle() {
        let err = parse("<|hel|lo|>").unwrap_err();
        assert!(matches!(err, ParseError::CursorNotAtBoundary { .. }));
    }

    #[test]
    fn error_selection_missing_cursor() {
        let err = parse("<|hello|>").unwrap_err();
        assert!(matches!(err, ParseError::SelectionMissingCursor { .. }));
    }
