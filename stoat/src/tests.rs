...
#[gpui::test]
fn open_command_palette_creates_state(cx: &mut TestAppContext) {
    let mut stoat = Stoat::test(cx);
    ...
    stoat.update(|s, cx| {
        ...
        let keymap = crate::keymap::create_default_keymap();
...
#[gpui::test]
fn command_palette_dismiss_clears_state(cx: &mut TestAppContext) {
    let mut stoat = Stoat::test(cx);
    ...
    stoat.update(|s, cx| {
        ...
        assert_eq!(s.mode(), "normal");
        assert!(s.command_palette_input().is_none());
        assert_eq!(s.command_palette_filtered().len(), 0);
        assert_eq!(s.command_palette_selected(), 0);
    });
}

#[gpui::test]
fn command_palette_next_increments(cx: &mut TestAppContext) {
    let mut stoat = Stoat::test(cx);
    ...
    stoat.update(|s, cx| {
        ...
        let command_count = s.command_palette_filtered().len();
        assert!(command_count > 1, "Need at least 2 commands for this test");

        assert_eq!(s.command_palette_selected(), 0);

        s.command_palette_next(cx);
        assert_eq!(s.command_palette_selected(), 1);
    });
}

#[gpui::test]
fn command_palette_prev_decrements(cx: &mut TestAppContext) {
    let mut stoat = Stoat::test(cx);
...
#[gpui::test]
fn command_palette_insert_filters(cx: &mut TestAppContext) {
    let mut stoat = Stoat::test(cx);
    ...
    stoat.update(|s, cx| {
        ...
        let initial_count = s.command_palette_filtered().len();

        // Insert text to filter (search for "Move" commands)
        s.insert_text("mov", cx);

        let filtered_count = s.command_palette_filtered().len();

        // Selection should reset to 0 after filtering
        assert_eq!(s.command_palette_selected(), 0);

        // Should have filtered the list
        assert!(filtered_count <= initial_count);
        assert!(filtered_count > 0, "Should have at least one Move command");
    });
}

#[gpui::test]
fn command_palette_backspace_refilters(cx: &mut TestAppContext) {
    let mut stoat = Stoat::test(cx);
    ...
    stoat.update(|s, cx| {
        ...
        s.insert_text("xyz", cx);
        let after_insert = s.command_palette_filtered().len();

        // Delete one character
        s.delete_left(cx);

        // Should re-filter with "xy"
        let after_delete = s.command_palette_filtered().len();

        // Should have reset selection
        assert_eq!(s.command_palette_selected(), 0);

        // Count likely increased (fewer filters means more results)
        assert!(after_delete >= after_insert);
    });
}

#[gpui::test]
fn command_palette_actions_noop_outside_mode(cx: &mut TestAppContext) {
    let mut stoat = Stoat::test(cx);
...
#[gpui::test]
fn command_palette_preserves_previous_mode(cx: &mut TestAppContext) {
    let mut stoat = Stoat::test(cx);
