...
impl GitRepo for LocalGitRepo {
    ...
    fn stash_create(&self) -> Option<String> {
        ...
        let index_tree = repo.find_tree(index_tree_oid).ok()?;

        let pathspecs: [&str; 1] = ["."];
        index
            .add_all(pathspecs.iter(), git2::IndexAddOption::DEFAULT, None)
            .ok()?;
        let workdir_tree_oid = match index.write_tree_to(&repo) {
            Ok(oid) => oid,
            Err(_) => {
        ...
        let workdir_tree = repo.find_tree(workdir_tree_oid).ok()?;
        if workdir_tree.id() == head_tree.id() {
            return None;
        }

        let sig = git2::Signature::now("stoat", "stoat@checkpoint").ok()?;
        let i_msg = format!("index on {branch_name}: {head_short} {summary}");
        let i_oid = repo
            .commit(None, &sig, &sig, &i_msg, &index_tree, &[&head_commit])
            .ok()?;
        let i_commit = repo.find_commit(i_oid).ok()?;

        let stash_msg = format!("WIP on {branch_name}: {head_short} {summary}");
        let stash_oid = repo
            .commit(
                None,
                &sig,
