...
use gpui::{App, IntoElement, RenderOnce, ScrollHandle, Window};
...
impl RenderOnce for GitStatus {
    fn render(self, _window: &mut Window, _cx: &mut App) -> impl IntoElement {
        ...
        if let Some(ref branch_info) = self.branch_info {
            metadata_lines.push(format!("On branch {}", branch_info.branch_name));
