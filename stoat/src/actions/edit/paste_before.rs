...
impl Stoat {
    /// Paste clipboard contents at the cursor position (before cursor).
    pub fn paste_before(&mut self, cx: &mut Context<Self>) {
        ...
        for selection in &selections {
            let offset = snapshot.point_to_offset(selection.head());
            edits.push((offset..offset, clipboard_text.as_str()));
        }
...
mod tests {
    ...
    #[gpui::test]
    fn paste_before_is_noop_without_clipboard(cx: &mut TestAppContext) {
