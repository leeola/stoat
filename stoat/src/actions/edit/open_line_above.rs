...
impl Stoat {
    ...
    pub fn open_line_above(&mut self, cx: &mut Context<Self>) {
        ...
        for selection in &selections {
            ...
            let line_start = snapshot.point_to_offset(text::Point::new(pos.row, 0));
            edits.push((line_start..line_start, "\n"));
        }
        ...
        for (i, selection) in selections.iter().enumerate() {
            let new_pos = text::Point::new(selection.head().row + i as u32, 0);
